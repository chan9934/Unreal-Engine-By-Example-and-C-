...
void AObstacleSpawner::Tick(float DeltaTime)
{
	...
	float trueSpawnTime = SpawnTimer / (float)GetCustomGameMode< ABountyDash_TestGameModeBase>(GetWorld())->GetGameLevel();
...
{
	if (SpawnTargets.Num() > 0 && ObstacleToSpawn.Num() > 0)
	{
		short Spawner = FMath::Rand() % SpawnTargets.Num();
		short Obstical = FMath::Rand() % ObstacleToSpawn.Num();
